...
\begin{figure}[tb]
\centering
	\includegraphics[width=0.80\linewidth]{../figuras/Ch2DoFControlSystem} 
...
The regulatory control main objective is \emph{load-disturbance rejection}; this is, to return the controlled variable to its set-point in the event a disturbance enters to the control system.  For the servo-control, it is intended to \emph{follow a changing set-point}; this its, to bring the controlled variable to its new desired value. Controller tuning for above operations must take also into account to not amplified the measurement noise, if any.  In figure (\ref{Ch2fig:CSTRExample}) we can see the CSRT example presented above, where a controller, is accomplishing the task of, first, tracking a set-point step change of ?5\%, followed by a disturbance attenuation  of two changes in the concentration, $C_{Ai}$, of $A$ in the feed flow of +20\% and finally a new change in the disturbance of ?20\%.

\begin{figure}[tb]
\centering
	\includegraphics[width=0.9\linewidth]{../figuras/Ch2CSRTExample} 
\caption{Control system operating for set-point following (servo) and disturbance attenuation (regulation} 
\label{Ch2fig:CSTRExample}
\end{figure}
