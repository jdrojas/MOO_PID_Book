...
%
One of the advantages of using a multiobjective problem approach, is its ability to take into account many cost functions at the same time, and being able to find the set of the ``best'' solutions. However, if the number of cost function is larger than two, using the data in the form of a plot may become cumbersome. In fact, for the cases of four or more functions it is impossible to directly plot the resulting Pareto.

To tackle these issues, different kind of plots have been proposed in the literature that tries to present all the information of the Pareto in a bi-dimensional graph.
